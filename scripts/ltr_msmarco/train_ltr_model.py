...
def train_data_loader(task='triple', neg_sample=20, random_seed=12345):
    ...
    if os.path.exists(f'./collections/msmarco-ltr-passage/train_{task}_sampled_with_{neg_sample}_{random_seed}.pickle'):
        ...
        if task == 'triple':
            ...
            with open(DIRECTION_QRELS, "r") as f:
                for line in f:
...
if __name__ == '__main__':
    ...
    sampled_train = train_data_loader(task='rank', neg_sample=args.neg_sample)
    ...
    print('IBM model Load takes %.2f seconds' % (end-start))
    start = end
    fe.add(IbmModel1("collections/msmarco-ltr-passage/ibm_model/text_bert_tok",
                     "text_bert_tok", "text_bert_tok", "text_bert_tok"))
    end = time.time()
    print('IBM model Load takes %.2f seconds' % (end-start))
    start = end
